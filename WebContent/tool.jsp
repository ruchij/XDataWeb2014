...
<%
  ...
  if(DEBUG == 0){	
	  ...
	  if ( "iitb.ac.in".equals(oauth_consumer_key) ) {
	    cons = new OAuthConsumer("http://moodle.iitb.ac.in/", "iitb.ac.in", "xd387sec", null);
	  ...
	  session.setAttribute("context_label", request.getParameter("context_label"));
  ...
  Connection dbcon = (new DatabseConnection()).dbConnection(properties.getHostname(),
		  properties.getDbName(), properties.getUsername1(), properties.getPasswd1());
	...
	if(!rs.next()){
		stmt = dbcon.prepareStatement("INSERT INTO users VALUES(?, ?, ?)");
